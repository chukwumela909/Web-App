...
This codebase is mid-migration between two backends. Know which one you're touching:

1. **External REST backend** (the newer path, on this branch). `src/lib/backend-api.ts` holds the `request`/`requestText` helpers and `BackendApiError`. Calls authenticate with the **Firebase ID token** (`user.getIdToken()`, sent as `Authorization: Bearer`), and responses are unwrapped from a `{ data }` / `{ error }` envelope. Base URL comes from `NEXT_PUBLIC_API_BASE_URL` (see `.env.example`; defaults to `https://fahampesa.amenviron.app/api/v1`, falling back to `http://localhost:4000/api/v1`). `src/lib/backend-business-api.ts` wraps business resources (products, sales, debtors, expenses, branches, suppliers, etc.) over that client. Note its `shouldUseFirebaseFallback` helper — several flows fall back to Firebase when the backend errors with a 5xx, so both paths can be live at once. Reports were recently wired to this API.

2. **Firebase** (Auth + Firestore, original path). Client SDK config is hardcoded in `src/lib/firebase.ts` (not env-driven — update that file to rotate). Firebase Admin bootstraps in `src/lib/firebase-admin-server.ts` from env secrets (`FIREBASE_PROJECT_ID`, `FIREBASE_PRIVATE_KEY`, `FIREBASE_CLIENT_EMAIL`) and **fails silently / falls back when missing** — guard for a null `adminAuth` like existing helpers do.
